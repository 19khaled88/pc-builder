...
export const Others = ({display}) => {
  const others = display.others

  const othersCard =(others) =>{
    let array =[]
    others.map((data)=>{
      array.push(
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5 p-2 ">{othersCard(others)}</div>
  )
}
