...
export const Monitor = ({display}) => {
    const monitor = display.monitor
    const monitorCard=(display)=>{
        let array =[]
        monitor.map((data)=>{
            array.push(
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5 p-2 ">{monitorCard(display)}</div>
  )
}
